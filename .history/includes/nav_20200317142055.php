<nav class="customstyle navbar navbar-expand-lg navbar-light bg-light">
    <a class="navbar-brand" href="#"><img src="V3.1-eksamen/img/LOGO-Starupuif.png" alt=""></a>
    <button class="navbar-toggler" type="button" data-toggle="collapse" data-target="#navbarSupportedContent" aria-controls="navbarSupportedContent" aria-expanded="false" aria-label="Toggle navigation">
        <span class="navbar-toggler-icon"></span>
    </button>
    <div class="collapse navbar-collapse" id="navbarSupportedContent">
        <ul class="navbar-nav mr-auto">
            <li class="nav-item">
                <a class="nav-link" href="index.php">Forside</a>
            </li>
            <li class="nav-item">
                <a class="nav-link" href="login.php">Log ind</a>
            </li>
            <li class="nav-item">
                <a class="nav-link" href="opretBruger.php">Opret Bruger</a>
            </li>
            <?php
            if (isset($_SESSION["loggedin"]))
            {
                echo "<li class='nav-item'>" . "<a class='nav-link' href=" . "loggedOut.php" . ">Log ud</a></li>";
                echo "<li>" . "<a class='nav-link href=" . "visBruger.php" . ">Min konto</a></li>";
            }
            else 
            {
                echo "<li>" . "<a class='nav-link href=" . "login.php" . ">Log ind</a></li>";
                echo "<li>" . "<a class='nav-link href=" . "opretBruger.php". ">Opret bruger</a></li>";
            }
        ?>
        </ul>
    </div>
</nav>